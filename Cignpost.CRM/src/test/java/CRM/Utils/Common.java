...
import static CRM.Utils.TestBase.logTestStep;
...
public class Common extends Start {
    ...
    public static void LoginUser(String UserName, String Password) throws Exception {
        ...
        $x("//span[text()='Log In']").waitUntil(Condition.visible,DEFAULT_WAIT).click();
        //ClickElement(By.id("M_Login"), "M_Login button");
        sleep(2000);
     ...
     */
      ...
      public static String getDocumentTypeToUpload(String fileType) throws AWTException {
        String FS = File.separator;
        String filePath = getProjectDirectory() + FS + "src" + FS + "test" + FS + "resources" + FS + "DocumentTypes";
          Robot rt=new Robot();
          rt.delay(2000);
    ...
    public static void PopUp(){
       // Boolean popup = $x("//div[@class='wm-close-button walkme-x-button']").isDisplayed();
        if($x("//div[@class='wm-close-button walkme-x-button']").isDisplayed())
        {
            ...
            $x("//div[@class='wm-close-button walkme-x-button']").click();
        }
        else {
